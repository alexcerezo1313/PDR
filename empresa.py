...
if 'competidors' not in st.session_state:
    st.session_state.competidors = []
if 'market' not in st.session_state:
    st.session_state.market = ""
if 'perfils' not in st.session_state:
    st.session_state.perfils = []
if 'delegations' not in st.session_state:
    st.session_state.delegations = 1
...
def pagina_requeriments_legals():
    ...
    st.markdown(
        "- **Societat Limitada (SL):** responsabilitat limitada al capital aportat.")
    st.markdown(
        "- **Societat Anònima (SA):** capital social més elevat, accions transferibles.")
    st.markdown(
        "- **Autònom:** responsabilitat il·limitada, ideal per activitats individuals.")
    st.markdown("**Permisos i llicències**: autoritzacions administratives per operar. Alguns exemples:")
    st.markdown(
        "- **Llicència d'obertura** per establiments.")
    st.markdown(
        "- **Permisos sanitaris** per activitats alimentàries.")
    st.markdown(
        "- **Autorització mediambiental** per activitats amb impacte.")
...
def pagina_capital():
    ...
    st.markdown(
        "- **Capital propi:** estalvis, reinversió dels beneficis.")
    st.markdown(
...
def pagina_escalabilitat():
    ...
    st.markdown(
        "Planificar la delegació consisteix a assignar tasques a responsables o nous departaments.")
    ...
    st.markdown(
        "L'ús de tecnologia (ERP, CRM, automatització) facilita l'expansió sense augmentar costos lineals.")
    if st.button("Continuar"):
        next_page()
